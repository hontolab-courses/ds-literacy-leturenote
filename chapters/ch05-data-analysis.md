...
- 推測統計と機械学習という「分析のその先」の世界を概観し，統計学や各学部の専門科目への見通しを持つ
...
この章では，まずデータを数字で要約する**記述統計**の基本を押さえ，次に2つの量の関係をとらえる**相関**を学び，そのうえで「相関があっても因果があるとは限らない」という落とし穴をじっくり掘り下げます．最後に，分析の世界のその先にある**推測統計**と**機械学習**の入口を覗きます．
...
アンスコムがこの例で伝えたかったのは，「**統計量による要約とグラフによる観察は，どちらか一方ではなく両方行うべきだ**」ということです {cite}`anscombe1973`．要約は便利ですが，要約の過程で必ず情報が失われます．データを分析するときは，統計量を計算するだけでなく，必ずグラフを描いて自分の目で分布を確かめる——この教訓は，50年経った今もデータ分析の鉄則であり続けています．グラフの描き方・読み方そのものは，次回（第6回）でじっくり扱います．
...
では，因果関係はどうすれば確かめられるのでしょうか．完全な答えは統計学の専門科目（因果推論と呼ばれる分野）に譲りますが，最も基本的な発想だけ紹介します．それは，**実際に原因を変えてみて（介入），変えなかった場合と比較する**というものです．

朝食の例で考えましょう．朝食を食べている学生と食べていない学生を「観察して比べる」だけでは，交絡（家庭環境など）の影響を取り除けません．そこで，たとえば次のような実験を考えます．

1. 参加者を集め，**くじ引き（ランダム）で**2つのグループに分ける
2. 一方のグループには朝食を提供し（介入するグループ），もう一方には今までどおり過ごしてもらう（比較するグループ）
3. 一定期間後に，2つのグループの成績を比較する

ポイントは「くじ引きで分ける」ところです．ランダムに分ければ，家庭環境も生活習慣も学力も，2つのグループの間でおおよそ均等にばらけます．つまり，交絡因子ごと2つのグループを「同じ条件」にそろえたうえで，朝食の有無**だけ**を変えられるのです．この状態で成績に差が出れば，それは朝食の効果だと考えてよい，という理屈です．このような方法は**ランダム化比較試験**（RCT）と呼ばれ，新薬の効果の確認などで実際に使われています．医療のニュースで「治験」という言葉を聞いたことがあるかもしれませんが，あれはまさにこの発想の実践です．
...
もちろん，実社会では倫理的・費用的に実験できないことも多くあります（「喫煙が健康に与える影響」を調べるために，くじ引きで人に喫煙させるわけにはいきません）．実験ができない状況で観察データから因果に迫るための方法論も発展していますが，それはこの科目の範囲を超えるので，「そういう分野がある」ということだけ知っておいてください．
...
## 分析のその先へ：推測統計と機械学習の入口

ここまで学んできた記述統計と相関は，「**手元にあるデータ**を要約し，関係をとらえる」ための道具でした．この章の締めくくりとして，その先に広がる2つの世界を覗いておきましょう．どちらも「入口」の紹介にとどめます．

### 推測統計の入口：一部から全体を推し量る

第2回（データの収集）で，**母集団**（知りたい対象の全体）と**標本**（実際に集められたその一部）という言葉を学んだのを覚えているでしょうか．世論調査が数千人への調査から有権者全体の支持率を推し量るように，私たちが手にできるデータはほとんどの場合，全体のごく一部です．

**手元の標本から，その背後にある母集団の性質を推し量る**ための統計学の分野を**推測統計**（inferential statistics）と呼びます．鍋いっぱいのみそ汁の味を，よくかき混ぜてから一口味見して判断する——推測統計がやっていることは，本質的にはこれと同じです．よくかき混ぜてあれば（＝標本に偏りがなければ），一口で全体の味がかなり正確に分かります．逆に，かき混ぜずに上澄みだけを味見すれば判断を誤ります．第2回で学んだ「標本の偏り」が，ここで効いてくるわけです．
...
推測統計には，「一部しか見ていないことによる誤差はどれくらいか」を見積もる道具（信頼区間）や，「観察された差は偶然の範囲か，それとも意味のある差か」を判定する道具（仮説検定）が用意されています．これらの中身は，統計学の入門科目で本格的に学ぶことになります．この章では，「**記述統計＝手元のデータを記述する**」「**推測統計＝手元のデータから全体を推し量る**」という役割の違いをつかんでおけば十分です．

### 機械学習の入口：予測というタスク

もう1つの世界が**機械学習**（machine learning）です．機械学習とは，人間がルールを1つ1つ書き込む代わりに，**大量のデータからコンピュータにパターンを学ばせて，未知のデータに対する予測や判別を行わせる**技術の総称です．

実は，みなさんは機械学習の恩恵を毎日受けています．

- メールアプリが迷惑メールを自動で振り分ける（過去の迷惑メールのパターンを学習）
- 動画サイトやネットショッピングの「あなたへのおすすめ」（似た行動をした人のデータから好みを予測）
- スマートフォンの顔認証（顔画像の特徴を学習して本人かどうかを判別）

第1回で紹介した医療の画像診断支援や交通の需要予測も，機械学習の応用です．

機械学習の中心的なタスクは**予測**です．そして予測の材料になっているのは，突き詰めれば，この章で学んだ「変数と変数の間の相関（関係のパターン）」です．ここで面白いのは，**予測が目的なら，因果がなくても相関さえあれば役に立つ場合がある**ということです．たとえば「傘を持って歩く人が多い日は，その日の遅くに道路が渋滞しがちだ」というパターンがあれば，傘が渋滞の原因でなくても（本当の原因は雨でも），渋滞の予測には使えます．一方で，「では渋滞を減らすにはどうすればよいか」という**介入**の問いに答えるには，因果の理解が必要になります {cite}`pearl2019`．予測には相関で足りることがあるが，対策（介入）には因果がいる——この区別は，AIの結果を社会で使うときの重要な視点です．機械学習と生成AIの関係については，第8回であらためて扱います．
...
- **相関があっても因果があるとは限りません**．偶然の一致（**疑似相関**），隠れた第3の変数（**交絡**），因果の向きの取り違えという3つの落とし穴があります．因果に迫る基本の発想は，ランダムに分けたグループへの**介入と比較**です．
- **推測統計**は標本から母集団を推し量る分野，**機械学習**はデータからパターンを学んで予測を行う技術です．予測には相関で足りる場合もありますが，対策（介入）を考えるには因果の理解が必要です．

この章で扱った内容は，いずれも入口にすぎません．代表値・ばらつき・相関係数の計算の理屈や，信頼区間・仮説検定の使い方は統計学の入門科目で，因果推論や実験計画の方法論は社会調査法・実験計画法・疫学・計量経済学など各学部の専門科目で，機械学習の仕組みは情報系の科目で，それぞれ本格的に学ぶことができます．この章が，それらの科目に出会ったときの「地図」になれば幸いです．

そして，アンスコムの四重奏が教えてくれたように，分析とグラフは切っても切れない関係にあります．次回（第6回）は，データを「見せる」技術と，グラフにだまされないための技術——データの可視化とコミュニケーション——を学びます．
